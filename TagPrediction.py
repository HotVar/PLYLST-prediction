...
train = train[train['tags'].apply(lambda x:len(x)==10)]  # tags가 10개인 데이터만을 사용
train = train[:5000]                                     # 5000개 학습 데이터 샘플 사용
n_tags = 500                                             # 최다 등장 500개 tags만을 사용
n_titles = 500                                           # 최다 등장 500개 title 형태소만을 사용
undup_tags = np.array((pd.Series(np.concatenate(train['tags'].values)).value_counts()[:n_tags].index))
...
concat_mat = pd.concat([train, test], axis=0)[['id', 'tags']]
...
np.save('test_tags_pred.npy', pred)
print(p_err)
print(c_err)
print(reg)
print(loss)